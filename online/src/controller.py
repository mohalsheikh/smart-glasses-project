...
class MainController:
    ...
    def _is_wake_word_only(self, text: str) -> bool:
        """Check if the user just said the wake word without a command."""
        t = (text or "").strip().lower()
        if not t:
            return False
        wake_phrases = [
            "hey vision", "vision", "hey glasses", "okay vision", "ok vision",
            "a vision", "hey visions", "hey fishing", "hey vishion",
        ]
        # Check if the text is ONLY a wake word (no command after it)
    ...
    def _is_currency_command(self, text: str) -> bool:
        t = (text or "").strip().lower()
        if not t:
            return False
        keywords = [
            "how much money", "identify money", "identify currency",
            "check money", "check currency", "what money", "count money",
            "how much is this", "what bills", "what bill",
            "read the money", "read money", "scan money",
            "how much cash", "count the money", "count the bills",
            "money check", "currency check",
        ]
        return any(k in t for k in keywords)
    ...
    def _is_doc_command(self, text: str) -> bool:
        ...
        # Also match "yes" / "yeah" if we just offered a full read
        if self.doc_reader.state and self.doc_reader.state.was_summarized and not self.doc_reader.state.user_wants_full_read:
            if t in {"yes", "yeah", "yep", "sure", "please", "yes please", "go ahead", "do it", "okay", "ok"}:
                return True
    ...
    def _run_doc_command(self, text: str, frame_snapshot) -> Optional[str]:
        ...
        # "Yes" after summary → read full text
        if self.doc_reader.state and self.doc_reader.state.was_summarized and not self.doc_reader.state.user_wants_full_read:
            if t in {"yes", "yeah", "yep", "sure", "please", "yes please", "go ahead", "do it", "okay", "ok"}:
                return self.doc_reader.read_full()

        # "Read everything" / "read full" / "read it all"
        if any(k in t for k in ["read everything", "read the full", "read full text", "read it all", "read all of it"]):
            if self.doc_reader.state:
                return self.doc_reader.read_full()
            else:
                msg = self.doc_reader.start(frame_snapshot, mode=self._get_read_mode())
                # If it was summarized, also read full
                if self.doc_reader.state and self.doc_reader.state.was_summarized:
                    return self.doc_reader.read_full()
                return msg
    ...
    def _handle_voice_interaction(self, frame) -> None:
        ...
        # Wait for audio to settle before starting mic recording
        time.sleep(0.3)
        ...
        def worker():
            ...
            try:
                ...
                # 0) Wake word detection — respond naturally
                if self._is_wake_word_only(text):
                    command_type = "wake_word"
                    self._speak_blocking(
                        "Hey! I'm right here. What do you need?",
                        meta={"req_id": req_id, "source": "voice"},
                    )
                    # Listen again for the actual command
                    t0b = time.perf_counter()
                    text2 = self.voice_listener.listen_and_transcribe()
                    listen_ms2 = (time.perf_counter() - t0b) * 1000.0
                    if not text2:
                        return
                    text = text2  # Replace with actual command
                    print(f"🎤 Follow-up command: {text!r}")

                # 0b) Currency commands — smart money identification
                if self._is_currency_command(text):
                    command_type = "currency"
                    if self.telemetry is not None:
                        self.telemetry.log_event("voice_command", {"req_id": req_id, "cmd": "currency"})
                    if frame_snapshot is not None:
                        result = self.currency.recognize(frame_snapshot)
                    else:
                        result = "I need to see the money. Hold it up to the camera."
                    self._speak_blocking(result, meta={"req_id": req_id, "source": "voice"})
                    return

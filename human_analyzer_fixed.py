...
#         # Tracking / de-dup tuning (defaults aimed at real-time stability)
#         draw_track_age_s: float = 0.20,
#         max_track_age_s: float = 0.55,
#         track_iou_threshold: float = 0.30,
#         track_center_threshold: float = 120.0,
#         nms_iou_threshold: float = 0.60,
#         min_face_height_px: int = 90,
#         min_hands_height_px: int = 80,
...
#         # Stable internal track ids (used when YOLO doesn't provide .boxes.id)
#         self._next_track_id: int = 0
        
#         # Tracking / de-dup tuning
#         self.draw_track_age_s = float(draw_track_age_s)
#         self.track_iou_threshold = float(track_iou_threshold)
#         self.track_center_threshold = float(track_center_threshold)
#         self.nms_iou_threshold = float(nms_iou_threshold)
#         self.min_face_height_px = int(min_face_height_px)
#         self.min_hands_height_px = int(min_hands_height_px)
...
#                         # De-duplicate pose detections (prevents double people overlays)
#                         cand_idx: List[int] = []
...
#                             cand_scores.append(float(conf[i]) if conf is not None else 1.0)
    
#                         keep = self._nms_xyxy(cand_boxes, cand_scores, self.nms_iou_threshold)
#                         for k in keep:
#                             i = cand_idx[k]
#                             x1, y1, x2, y2 = cand_boxes[k]
#                             raw_tid = int(ids[i]) if ids is not None and i < len(ids) else None
#                             if raw_tid is not None:
#                                 track_id = raw_tid
#                             else:
#                                 match_tid = self._match_track_for_detection((x1, y1, x2, y2), seen_ids, now, w, h)
...
#                             body = self._tracked_bodies.get(int(track_id))
#                             if body is None:
#                                 body = BodyData(first_seen=now)
#                                 body.track_id = int(track_id)
#                                 self._tracked_bodies[int(track_id)] = body
...
#                 if (body.last_seen is not None) and (now - body.last_seen <= self.draw_track_age_s):
#                     self._predict_body_motion(body)
#                     body.time_tracked = now - body.first_seen
#                     bodies.append(body)
...
#         # ─────────────────────────────────────────────────────────────────────
#         # Gate expensive detectors by subject size (big perf win + less false positives)
#         if run_hands and bodies and not any(((b.height_px or 0) >= self.min_hands_height_px) for b in bodies):
#             run_hands = False
#         if run_face and bodies and not any(((b.height_px or 0) >= self.min_face_height_px) for b in bodies):
#             run_face = False
...
#     @staticmethod
#     def _nms_xyxy(boxes_xyxy: List[Tuple[int, int, int, int]], scores: List[float], iou_thresh: float) -> List[int]:
#         """Simple non-max suppression to prevent duplicate people on top of each other."""
#         if not boxes_xyxy:
#             return []
...
#             remaining = []
#             for j in idxs:
#                 if HumanAnalyzer._box_iou(boxes_xyxy[cur], boxes_xyxy[j]) < iou_thresh:
#                     remaining.append(j)
#             idxs = remaining
#         return keep

#     def _match_track_for_detection(
#         self,
...
#         frame_h: int,
#     ) -> Optional[int]:
#         """Match a detection bbox to an existing track id (when YOLO provides no stable IDs)."""
#         if not self._tracked_bodies:
#             return None
...
#         max_center_dist = self.track_center_threshold * diag

#         best_tid: Optional[int] = None
#         best_score = -1e9

#         for tid, b in self._tracked_bodies.items():
#             if tid in seen_ids:
#                 continue
#             if b.last_seen is None or (now - b.last_seen) > self.max_track_age_s:
#                 continue

#             bb = b.bbox_smooth or b.bbox
#             if bb is None:
#                 continue

#             iou = self._box_iou(bbox, (int(bb[0]), int(bb[1]), int(bb[2]), int(bb[3])))
#             bc = b.center_smooth or b.center
#             if bc is None:
#                 continue
#             dist = ((cx - bc[0]) ** 2 + (cy - bc[1]) ** 2) ** 0.5

#             score = (iou * 2.5) - (dist / max(1.0, max_center_dist))

#             if (iou >= self.track_iou_threshold) or (dist <= max_center_dist * 0.15):
#                 if score > best_score:
#                     best_score = score
#                     best_tid = tid
